...
export default function MyAnswersPage() {
    ...
    useEffect(() => {
        const fetchLessons = async () => {
            try {
                const data = await testService.getLessons();
    ...
    const fetchMyAnswers = async () => {
        try {
            const data = await testService.getMyAnswers({
            ...
            // Backenddan keladigan totalPages va totalElements'ni to'g'ri o'qish
    ...
    useEffect(() => {
        fetchMyAnswers();
    }, [page, filterLessonId, filterIsCorrect]);

    return (
        <div className="space-y-4 md:space-y-6 p-2 md:p-6 max-w-7xl mx-auto">
            <div>
                <h1 className="text-xl md:text-2xl font-bold text-[#1E1B3A]">Mening test javoblarim</h1>
                <p className="text-xs text-[#6B6B6F]">Bajarilgan testlar va tarjima natijalari tarixi</p>
            </div>
            ...
            <div className="bg-white p-3 md:p-4 rounded-2xl border border-[#E8DEFF] shadow-sm flex flex-col sm:flex-row gap-3 items-center">
                <select 
                    ...
                    className="px-3 py-2 border border-[#E8DEFF] rounded-lg text-sm w-full sm:w-72 focus:outline-none focus:border-[#5B3DF5] bg-white text-[#1E1B3A]"
                >
            ...
            <div className="bg-white rounded-2xl border border-[#E8DEFF] shadow-sm overflow-hidden">
                            ...
                            {answers.length > 0 ? (
                                answers.map((item) => (
                                    <tr key={item.id} className="hover:bg-[#F8F7FF]/50 transition">
                                        <td className="p-3 md:p-4 font-medium">{item.lessonName || item.lesson_name}</td>
                                        <td className="p-3 md:p-4 text-center">
                                            <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                                                (item.isCorrect ?? item.is_correct) ? 'bg-[#22C55E]/10 text-[#22C55E]' : 'bg-[#EF4444]/10 text-[#EF4444]'
                                            }`}>
                                                {(item.isCorrect ?? item.is_correct) ? "To'g'ri" : "Noto'g'ri"}
                                            </span>
                                        </td>
                                ...
                                <tr>
                                    <td colSpan="3" className="p-6 text-center text-gray-400 text-sm">Ma'lumot topilmadi</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                ...
                {/* --- PAGINATION (PC va Phone uchun mukammal moslashtirilgan) --- */}
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 max-w-md w-full space-y-4 shadow-xl border border-[#E8DEFF]">
                        <div className="flex justify-between items-center border-b border-[#E8DEFF] pb-3">
                            <h3 className="font-bold text-lg text-[#1E1B3A]">Javob tafsilotlari</h3>
                        ...
                        <div className="space-y-3 text-sm text-[#1E1B3A]">
                            ...
                            <div className="bg-[#F8F7FF] p-3 rounded-xl border border-[#E8DEFF] space-y-2">
                                ...
                                <div>
                                    <span className="text-xs text-[#6B6B6F] block">Sizning tanlovingiz (RU / UZ):</span>
                            ...
                            <div>
                                ...
                                <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-bold mt-1 ${
                                    ...
                                    {(selectedDetail.isCorrect ?? selectedDetail.is_correct) ? "To'g'ri" : "Noto'g'ri"}
                                </span>
                            </div>
                        </div>
