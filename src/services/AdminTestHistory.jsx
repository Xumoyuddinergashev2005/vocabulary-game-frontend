...
export default function AdminTestHistory() {
  ...
  // Qidiruv bo'yicha harflar bosilganda darhol ishlamasligi uchun useEffect'ga search'ni qo'shmadim. 
  // Qidiruv tugmasini bossa yoki Enter bossa ishlatsa bo'ladi. Hozircha oddiy holatda qoldirildi.

  const loadHistory = async () => {
    try {
      setLoading(true);
      const data = await testService.getTestHistory(filters);
      // Spring Data JPA odatda page.content qaytaradi
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50/50 border-b border-gray-100 text-gray-500 text-xs uppercase tracking-wider">
              <th className="p-4 font-semibold">Foydalanuvchi ID</th>
          ...
          <tbody className="divide-y divide-gray-50 text-sm text-gray-700">
            {loading ? (
              ...
              history.map((item) => (
                <tr key={item.session_id || item.id} className="hover:bg-gray-50/80 transition">
                  <td className="p-4 font-medium text-gray-900">
                    User #{item.user_id}
                  </td>
                  <td className="p-4 text-gray-600 font-medium">{item.lesson_name}</td>
                  <td className="p-4">
                    <div className="flex flex-col gap-1 items-start">
                      ...
                      <span className="text-[11px] text-gray-400 font-medium">{item.attempt_type}</span>
                    </div>
                  </td>
                  <td className="p-4 font-bold text-gray-800">
                    <span className="text-green-600">{item.correct_answers || 0}</span> / {item.total_questions || 0}
                  </td>
